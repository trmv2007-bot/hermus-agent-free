...
def _ok_transport():
    class _T:
        def connect(self):
            return {"ok": True, "device": "abc"}

        def get_screen(self, **kw):
            return {"ok": True, "format": "png", "bytes": 100, "data": "x"}

        def get_ui_tree(self, **kw):
            return {"ok": True, "format": "xml", "data": "<node/>"}
        ...
        def launch_app(self, package, **kw):
...
def test_adb_transport_screencap_returns_valid_png_bytes():
    from core.android.transport import PNG_SIGNATURE, AdbAndroidTransport
    from tests._android_fixtures import PNG_1x1_BYTES

    calls = []
    t = AdbAndroidTransport(adb="/usr/bin/adb", runner=_fake_runner(calls, binary=True))
    r = t.get_screen()
    assert r["ok"] is True
...
def test_adb_transport_rejects_corrupt_screencap():
    ...
    t = AdbAndroidTransport(adb="/usr/bin/adb", runner=_fake_runner(calls, stdout="not a png"))
    with pytest.raises(AndroidUnavailable, match="valid PNG"):
        t.get_screen()
    # Empty output must also fail.
    t2 = AdbAndroidTransport(adb="/usr/bin/adb", runner=_fake_runner(calls, stdout=""))
...
def test_audit_records_ops_and_emits_event(tmp_path):
    ...
    try:
        ...
        assert any(e.command == "tap" and e.source == "android" and e.status == "ok" for e in snap), (
            "android op must be mirrored onto the canonical EventBus"
        )
    finally:
        bus.close()
...
def test_default_transport_factory_provisions_adb_when_available(monkeypatch):
    ...
    from core.android.transport import AdbAndroidTransport, build_default_transport
...
def test_default_transport_factory_bridge_when_configured(monkeypatch):
    from core.android.transport import BridgeAndroidTransport, build_default_transport

    monkeypatch.setenv("HERMUS_ANDROID_TRANSPORT", "bridge")
    monkeypatch.setenv("HERMUS_ANDROID_BRIDGE_URL", "http://127.0.0.1:8080")
    import base64

    monkeypatch.setenv("HERMUS_ANDROID_SECRET", base64.urlsafe_b64encode(b"x" * 32).decode())
    t = build_default_transport()
...
def test_launch_app_supports_component_and_package(monkeypatch):
    ...
    assert [a for a in calls if "am" in a][-1] == ["/usr/bin/adb", "shell", "am", "start", "-n", "com.example.app/.MainActivity"]
    ...
    assert r2["ok"]
    assert ["/usr/bin/adb", "shell", "cmd", "package", "resolve-activity", "--brief", "com.example.app"] in calls
...
def test_bridge_transport_requires_loopback_or_tls():
    from core.android.transport import AndroidUnavailable, BridgeAndroidTransport
    ...
    with pytest.raises(AndroidUnavailable, match="loopback|https"):
        BridgeAndroidTransport(base_url="http://evil.example.com:8080", secret=b"x" * 32)
    with pytest.raises(AndroidUnavailable, match="loopback|https"):
...
def test_secure_sign_verify_and_pairing():
    from core.android.secure import new_pairing_secret, pairing_response, sign, verify, verify_pairing
