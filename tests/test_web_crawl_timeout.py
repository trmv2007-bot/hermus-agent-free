...
def _ok_result(url: str) -> WebResult:
    return WebResult(
        ...
        status_code=200,
        title="t",
        text="plenty of body text here " * 5,
        strategy="static",
        size_bytes=200,
        sha256="x",
    )
...
class TestWallClockDeadline:
    def test_timeout_is_reported_accurately(self):
        ...
        plan = plan_crawl(
            urls, requested_pages=4, requested_depth=0, requested_concurrency=1, per_domain_delay_ms=0, max_pages_ceiling=10
        )
        router = SlowRouter(0.25)
        worker = CrawlWorker(plan, router=router, policy=_policy(), wall_clock_seconds=0.4)
        summary = worker.run()
    ...
    def test_no_new_work_scheduled_after_deadline(self):
        ...
        plan = plan_crawl(
            urls, requested_pages=8, requested_depth=0, requested_concurrency=1, per_domain_delay_ms=0, max_pages_ceiling=10
        )
        router = SlowRouter(0.25)
        worker = CrawlWorker(plan, router=router, policy=_policy(), wall_clock_seconds=0.3)
        started = time.monotonic()
        summary = worker.run()
        elapsed = time.monotonic() - started
        assert len(router.calls) <= 3, f"work kept being scheduled after the deadline: {router.calls}"
        assert summary["timed_out"] is True
    ...
    def test_rate_limit_wait_is_deadline_bounded(self):
        ...
        plan = plan_crawl(
            urls, requested_pages=2, requested_depth=0, requested_concurrency=1, per_domain_delay_ms=10_000, max_pages_ceiling=5
        )
    ...
    def test_cancellation_interrupts_rate_limit_wait(self):
        ...
        worker = CrawlWorker(
            plan, router=router, policy=_policy(), should_cancel=lambda: time.monotonic() >= flip_at, wall_clock_seconds=60.0
        )
        started = time.monotonic()
        summary = worker.run()
        elapsed = time.monotonic() - started
    ...
    def test_inflight_fetch_is_abandoned_not_killed(self, monkeypatch):
        ...
        plan = plan_crawl(
            urls, requested_pages=1, requested_depth=0, requested_concurrency=1, per_domain_delay_ms=0, max_pages_ceiling=5
        )
        router = SlowRouter(5.0)  # far longer than deadline + grace
        worker = CrawlWorker(plan, router=router, policy=_policy(), wall_clock_seconds=0.2)
        started = time.monotonic()
        summary = worker.run()
        elapsed = time.monotonic() - started
    ...
    def test_generous_deadline_changes_nothing(self):
        ...
        urls = ["http://x.example/a", "http://x.example/b"]
        plan = plan_crawl(
            urls, requested_pages=2, requested_depth=0, requested_concurrency=2, per_domain_delay_ms=0, max_pages_ceiling=5
        )
        router = SlowRouter(0.0)
        worker = CrawlWorker(plan, router=router, policy=_policy(), wall_clock_seconds=30.0)
        summary = worker.run()
...
class TestJoinBudgetIsGlobal:
    ...
    def test_many_stuck_threads_share_one_join_budget(self, monkeypatch):
        ...
        worker = CrawlWorker(plan, router=SlowRouter(0.0), policy=_policy(), wall_clock_seconds=0.0)
    ...
    def test_join_waits_are_not_cumulative_as_threads_grow(self, monkeypatch):
        ...
        assert eight < two + 0.4, f"wait scaled with thread count ({two:.2f}s → {eight:.2f}s)"
    ...
    def test_early_finishers_still_join_normally(self, monkeypatch):
        ...
        worker = CrawlWorker(plan, router=SlowRouter(0.0), policy=_policy(), wall_clock_seconds=5.0)
        threads = [threading.Thread(target=time.sleep, args=(0.05,), daemon=True) for _ in range(4)]
        for t in threads:
            t.start()
...
class TestGatewayWallClock:
    def test_gateway_honors_requested_wall_clock_bounded_by_ceiling(self, monkeypatch):
        ...
        gateway.crawl(["http://127.0.0.1:9/"], max_pages=1, max_depth=0, wall_clock_seconds=7.5)
        assert seen["wall_clock_seconds"] == 7.5

        gateway.crawl(["http://127.0.0.1:9/"], max_pages=1, max_depth=0, wall_clock_seconds=99_999.0)
        assert seen["wall_clock_seconds"] == 30.0, "requested wall clock must be clamped to the configured ceiling"
