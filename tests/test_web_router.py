...
def make_raw(url: str, *, html: str = "", status: int = 200, text: str | None = None):
...
class TestPlan:
    ...
    def test_unavailable_dynamic_is_dropped(self, router, monkeypatch):
        ...
        assert all(statuses[s.value] in (capabilities.AVAILABLE, capabilities.NOT_VERIFIED) for s in plan)
    ...
    def test_termux_restriction_removes_browser_strategies(self, monkeypatch):
        ...
        plan = router.plan("auto")
        assert [s.value for s in plan] == ["static"], "Termux default must restrict to lightweight HTTP (untested browsers)"
...
class TestEscalation:
    def _patch_router_backend(self, monkeypatch, router, behaviors: dict):
        ...
        monkeypatch.setattr(router, "_run_strategy", lambda strat, url, **kw: make_run(strat.value)(strat.value, url, **kw))
    ...
    def test_insufficient_static_escalates_to_dynamic(self, monkeypatch):
        ...
        behaviors = {
            "static": lambda url: make_raw(
                url, html="<html><head><title>t</title></head><body><div id='root'></div></body></html>", text=""
            ),
            "dynamic": lambda url: make_raw(
                url, html="<html><body>rendered content that is long enough to be sufficient here</body></html>"
            ),
        }
    ...
    def test_failure_does_not_retry_same_strategy(self, router, monkeypatch):
        ...
        def static_down(url):
            calls.append("static")
            raise WebAcquisitionError(
                "connection refused", failure_class=FailureClass.CONNECTION, error_code="WEB_CONNECTION_ERROR", retryable=True
            )
    ...
    def test_forced_strategy_without_fallback_runs_alone(self, monkeypatch):
        ...
        def fake_run(strat, url, **kw):
            ...
            raise WebAcquisitionError("down", failure_class=FailureClass.CONNECTION, error_code="WEB_CONNECTION_ERROR")
...
class TestNoFallbackContract:
    ...
    def test_auto_no_fallback_never_escalates_past_js_shell(self, monkeypatch):
        ...
        calls: list[str] = []
        behaviors = {
            "static": lambda url: make_raw(url, html="<html><body><div id='root'></div></body></html>", text=""),
            "dynamic": lambda url: make_raw(url, html="<html><body>rendered content long enough here</body></html>"),
        }
        ...
        monkeypatch.setattr(router, "_run_strategy", fake_run)
        result = router.fetch("https://spa.example/", strategy="auto", allow_fallback=False)
        assert calls == ["static"], "auto + allow_fallback=False must not escalate"
        assert result.ok is False
        assert [a.strategy for a in result.attempts] == ["static"]
        assert any("insufficient" in w for w in result.warnings), "must say honestly that content was fetched but insufficient"
    ...
    def test_auto_no_fallback_never_escalates_past_transport_failure(self, monkeypatch):
        ...
        def fake_run(strat, url, **kw):
            ...
            if strat.value == "static":
                raise WebAcquisitionError(
                    "connection refused", failure_class=FailureClass.CONNECTION, error_code="WEB_CONNECTION_ERROR"
                )
            return make_raw(url, html="<html><body>would have worked</body></html>")

        monkeypatch.setattr(router, "_run_strategy", fake_run)
        result = router.fetch("https://down.example/", strategy="auto", allow_fallback=False)
    ...
    def test_auto_with_fallback_still_escalates(self, monkeypatch):
        ...
        monkeypatch.setattr(router, "_run_strategy", fake_run)
        result = router.fetch("https://spa.example/", strategy="auto", allow_fallback=True)
    ...
    def test_explicit_static_no_fallback_stays_static(self, monkeypatch):
        ...
        def fake_run(strat, url, **kw):
            calls.append(strat.value)
            return make_raw(url, html="<html><body><div id='root'></div></body></html>", text="")

        monkeypatch.setattr(router, "_run_strategy", fake_run)
        result = router.fetch("https://spa.example/", strategy="static", allow_fallback=False)
        assert calls == ["static"]
        assert result.ok is False
