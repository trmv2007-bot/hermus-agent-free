...
class RecordingRouter:
    ...
    def fetch(self, url, *, strategy="auto", want_markdown=False, allow_fallback=True, **kw):
        self.calls.append({"url": url, "strategy": strategy, "allow_fallback": allow_fallback})
        return WebResult(
            ok=True,
            url=url,
            final_url=url,
...
class TestCrawlStrategyPolicy:
    def test_auto_crawl_uses_auto_router_not_forced_static(self):
        plan = plan_crawl(["http://x.example/"], requested_pages=1, requested_depth=0, strategy="auto")
    ...
    def test_static_only_crawl_stays_static(self):
        plan = plan_crawl(["http://x.example/"], requested_pages=1, requested_depth=0, strategy="static")
        router = RecordingRouter()
        worker = CrawlWorker(plan, router=router, policy=_policy())
        worker.run()
    ...
    def test_dynamic_crawl_requests_dynamic(self):
        plan = plan_crawl(["http://x.example/"], requested_pages=1, requested_depth=0, strategy="dynamic")
...
class TestPerDomainDelayPropagation:
    ...
    def test_worker_honors_delay_between_same_domain_hits(self):
        ...
        plan = plan_crawl(
            urls, requested_pages=2, requested_depth=0, requested_concurrency=1, per_domain_delay_ms=300, max_pages_ceiling=5
        )
        router = RecordingRouter()
        worker = CrawlWorker(plan, router=router, policy=_policy())
    ...
    def test_zero_delay_is_fast(self):
        urls = ["http://x.example/a", "http://x.example/b"]
        plan = plan_crawl(
            urls, requested_pages=2, requested_depth=0, requested_concurrency=1, per_domain_delay_ms=0, max_pages_ceiling=5
        )
        router = RecordingRouter()
        worker = CrawlWorker(plan, router=router, policy=_policy())
        started = time.monotonic()
