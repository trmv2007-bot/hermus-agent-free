...
class TestLifecycle:
    def test_create_describe_no_cookie_values(self, manager):
        ...
        assert "cookies" not in info and "cookie" not in str(info).lower() or "cookie_values" not in info
...
class TestRealSessionFetch:
    ...
    def test_fetch_through_session_hits_real_server_and_reuses_client(self):
        ...
        class Handler(BaseHTTPRequestHandler):
            def do_GET(self):
                ...
                body = (
                    b"<html><head><title>Session Page</title></head><body>"
    ...
    def test_session_create_fails_typed_without_scrapling(self, monkeypatch):
        ...
        monkeypatch.setattr("scrapling.fetchers.FetcherSession", boom, raising=False)
