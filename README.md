...
### 5c. Retrieval, Async Gateway, Jails & Delegation Trees — The Architecture Round ⭐

- **Hybrid memory:** recall is now BM25 (FTS5) + dense vectors fused with Reciprocal Rank Fusion, plus the stored importance prior — `"postgres"` finds `"PostgreSQL connection pooling"`. FTS query sanitizing, sqlite-vec when installed, brute-force cosine otherwise, `LIKE` last. Every hit says *why* it matched (`retrieval.mode`, `bm25_rank`, `vector_rank`, `backend`) → `hermus mem2 hybrid "<query>" --explain`
- **Decay instead of rot:** exponential recency decay whose half-life lengthens with access, floored so nothing is silenced outright, banded `hot/warm/cold/archive/purge`. Pinned memories never decay. The memory block is packed to a token budget by value density with per-kind caps, so stale context cannot crowd out fresh context. `hermus mem2 context` shows the packed block + evictions, `hermus mem2 compact|sweep --apply|pin|forget`, background sweep every `HERMUS_MEMORY_SWEEP_MINUTES`
- **The agent forges its own skills:** after a *verified* turn, a ≥3-tool-call trajectory is distilled into `skills/<name>/SKILL.md` + an importable `skill.py`, smoke-tested, quarantined if invalid, deduped by cosine (merge, not copy), and re-scored by later outcomes — a skill that keeps failing stops being re-harvested. `hermus forge list|log|run`, `POST /skills/forge/harvest`, tools `skill_harvest`, `skill_forge_stats`
- **Queue first, never blocked:** `POST /command` and channel webhooks return `202` with `job_id`/`run_id` handles while asyncio workers run the job. One lane per `platform:user` (a user's turns never interleave), parallel across lanes, priorities, retry with backoff, per-job timeout that cancels cooperatively, `dedupe_key` for redeliveries, durable `data/jobs/jobs.jsonl` + `results/*.json` so history survives a restart and `hermus jobs list` works from another process. Optional Redis Streams (`HERMUS_QUEUE_BACKEND=redis`) — probed first, falls back to in-process instead of failing. `HERMUS_QUEUE_ENABLED=0` runs everything inline again
- **Live, resumable streaming:** `GET /stream/run/{run_id}` (SSE) and `WS /ws/agent` carry every step — `memory_recalled, step_started, llm_delta, tool_call, tool_result, verification, skill_created, run_finished` — with monotonic event ids, so `Last-Event-ID`/`?after=N` replays exactly what a reconnecting client missed. Real upstream token streaming via `stream:true` + `include_usage`. Cancel from any side: `POST /jobs/{id}/cancel`, `{"action":"cancel"}` over WS
- **Shell commands go to jail:** every local execution runs under a policy — CPU, memory, PID and disk caps, dropped capabilities, `no_new_privs`, read-only rootfs, `network=off` (proxies blackholed), env allowlist with secret scrubbing, deny-pattern pre-filter (`rm -rf /`, fork bombs, `curl | sh` → `rc=126`, unblocked only via audited `allow_dangerous`), capped and ANSI-stripped output, structured JSONL audit line per run. Backend ladder `docker → podman → bwrap → local(rlimits) → off`; `hermus sandbox status` tells you what it actually got, and `HERMUS_SANDBOX=off` is explicit rather than silent
- **Delegation is a tree, not a fire-and-forget:** the orchestrator forks children that talk JSON-RPC over stdio (`ping`, `agent.run`, `tool.call`, `memory.recall`, `cancel`), each returning a full contract — `answer, confidence, evidence, artifacts, tool_calls, usage, steps, error`. `fanout` runs workstreams in parallel, `decompose_and_run` plans them, `aggregate` merges (`concat/vote/best/synthesize`, with a confidence discount when children disagree), depth and worker budgets bound the blast radius, `cancel_tree` stops a subtree at the next step boundary. `hermus delegate "goal" --task … --aggregate best`, `POST /delegate`, `GET /delegation/{tree_id}`, tools `delegate_tasks` / `subagent_spawn`, `python -m core.delegation --self-test`
- **Design + contracts:** `ARCHITECTURE_UPGRADES.md` (Part II). Tests, all offline with `mock/mock` — no keys, no Docker, no Redis: `python tests/test_hybrid_memory.py`, `tests/test_skill_forge.py`, `tests/test_sandbox.py`, `tests/test_delegation.py`, `tests/test_gateway_realtime.py`
