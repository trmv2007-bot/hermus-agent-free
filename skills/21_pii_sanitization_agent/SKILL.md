...
description: Sanitizes PII from text before it reaches LLMs or external APIs, using the TrustBoost API — fail-closed, multilingual, on-chain proof - Use when user says 21 pii sanitization agent or needs general agent - Framework: other - From 500 AI Agents Projects (36k stars) - 21-pii-sanitization-agent
---

# pii-sanitization-agent - 21_pii_sanitization_agent

Sanitizes PII from text before it reaches LLMs or external APIs, using the TrustBoost API — fail-closed, multilingual, on-chain proof

**Framework:** other  
**Industry:** general  
**Original:** https://github.com/ashishpatel26/500-AI-Agents-Projects/tree/main/21-pii-sanitization-agent  
**Tags:** other, general, 500-agents, free

## What it does

Sanitizes PII from text before it reaches LLMs or external APIs, using the TrustBoost API — fail-closed, multilingual, on-chain proof

Original agent from 500-AI-Agents-Projects collection - 21-pii-sanitization-agent:

- Takes user query/task
- Uses other framework
- Produces structured output

## Original Setup (from 500-AI-Agents-Projects)

```bash
cd agents/21-pii-sanitization-agent
...
```
/21_pii_sanitization_agent
```

Or:

```
You: I need 21 pii sanitization agent
Hermus: [Tool] skill_use({"name": "21_pii_sanitization_agent"})
```
...
- "general agent"
- "I need Sanitizes PII from text before it reaches LLMs or "
...
This agent belongs to general industry. Other industries in collection:
...
If you want original exact code, clone: https://github.com/ashishpatel26/500-AI-Agents-Projects/tree/main/21-pii-sanitization-agent
