...
`.github/workflows/ci.yml` exists and runs `pytest` (3.10/3.11/3.12) +
`compileall` + `ruff --select E9,F63,F7,F82,B,PLE` on every push and PR.
`docs/ci-workflow.yml` is the reference copy. A green check is therefore a
GitHub result, not a local claim — verify any commit with:
