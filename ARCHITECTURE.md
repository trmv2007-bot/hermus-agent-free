...
| World state | ✅ **DONE** — `world_state_v2.py` **deleted**, construction now routed through `core.state` | `core/computer/world_state.WorldState` (v1) is canonical; `core.state` is the single public boundary. Production modules build world state via `core.state.create_world_state` / `world_state_from_dict` / `load_world_state` rather than instantiating `WorldState` directly (`ComputerAgent` and `TaskStore` migrated), and `WorldStateFacade` / `get_world_state` wrap one instance for callers that want the shared snapshot. The dead V2 duplicate (exported only from `__init__`, used by no functional code) was removed. Gates: `test_one_worldstate_owner` (one facade **and** no direct `WorldState(...)` outside the owner) + `test_worldstate_facade_is_reachable_from_production`. `migrate_world_state` no longer overwrites an unreadable file. |
