...
| Verification / capability | ✅ **integration + behavioral gates** | Canonical owners (MissionEngine/ModelGateway/ToolGateway/JobQueue/MemoryFacade/EventBus) drive realistic offline flow tests (`tests/test_capability_flows.py`, Tasks 1-8) and 25 architecture/proof gates (22 structural in `test_architecture_gates.py` + 3 behavioral), including **behavioral proof** gates (`tests/test_behavioral_boundary_proof.py`) that run a real agent turn and assert ModelGateway / ToolGateway / MemoryFacade were actually used. Full suite: **630 passed, 0 failed**. |
...
6. **Android control subsystem (Spec §16–19) — NOT YET BUILT.** Today there is no Android subsystem at all (no `android`/`AndroidTool`, no secure transport, no companion app). This is a major outstanding build and must not be reported as WORKING. Required: backend → `ToolGateway` → `AndroidTool` → secure local bridge → Android Agent Companion (accessibility/UI control, screen capture, device APIs, ADB, intents, notification access with user consent); explicit authorization + permission checks + audit + secure pairing + encrypted transport + configurable allowed ops; report `android_control_unavailable` when a device/missing permission is present. E2E must run on an emulator/device, not mocks.
7. **Restart/resume + host E2E (Spec §13)** — real mission persisted → kill/restart worker → restore → continue → verify; status/event-history/artifacts survive; duplicate execution prevented. Host-level computer E2E (launch app → observe → click → type → verify) still to be wired and proven on a real host.
