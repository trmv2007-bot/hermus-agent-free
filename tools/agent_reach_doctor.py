...
def probe_command(cmd: list[str], timeout: int = 10) -> dict:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if "No such file or directory" in result.stderr or "not found" in result.stderr.lower():
                return {
                    ...
                    "cmd": " ".join(cmd),
                    "fix": f"Install: {' '.join(cmd)} not found",
                }
            else:
                return {
                    "status": "broken",
        ...
        return {
            ...
            "cmd": " ".join(cmd),
            "fix": f"Install {cmd[0]}: pip install {cmd[0]} or apt install",
        }
        ...
        return {
            ...
            "cmd": " ".join(cmd),
            "fix": "Command timed out, maybe network issue, retry",
        }
...
def doctor_check_all() -> dict:
    ...
    # 3. RSS - feedparser (find_spec probes availability without importing)
    try:
        feedparser_available = importlib.util.find_spec("feedparser") is not None
    except (ImportError, ValueError):
        feedparser_available = False
    if feedparser_available:
        checks.append(
            {"platform": "rss", "name": "RSS", "status": "available", "backend": "feedparser", "zero_config": True, "free": True}
        )
    else:
        checks.append(
            {
                ...
                "status": "missing",
                "backend": "feedparser",
                "fix": "pip install feedparser (free)",
            }
        )
...
def doctor_text_report() -> str:
    ...
    lines.append(
        f"Available: {result['available']} | Partial/Fallback: {result['partial']} | Requires Config: {result['requires_config']} | Missing: {result['missing']}"
    )
    lines.append("")
    for check in result["checks"]:
        icon = (
            ...
            if check["status"] in ("partial", "fallback")
            else "❌"
        ...
        lines.append(
            f"   Status: {check['status']} | Backend: {check.get('active_backend', 'unknown')} | Candidates: {', '.join(check.get('backend_candidates', []))}"
        )
        lines.append(f"   Zero-config: {check.get('zero_config')} | Free: {check.get('free')}")
        if check.get("probe"):
            probe = check["probe"]
            lines.append(
                f"   Probe: {probe.get('cmd')} -> {probe.get('status')}: {probe.get('output', '')[:80] or probe.get('error', '')[:80]}"
            )
