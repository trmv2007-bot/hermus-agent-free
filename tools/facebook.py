...
def facebook_search(query: str, max_results: int = 5) -> dict:
    ...
    try:
        ...
        result["note"] = (
            "Free via Jina Reader for public Facebook pages. For full search/feed/groups, needs OpenCLI Chrome session free"
        )
        return result
...
def instagram_user_search(username: str) -> dict:
    ...
    try:
        ...
        return {
            "success": False,
            "error": str(e),
            "config_guide": "Tell Agent 'help me config Instagram' - OpenCLI Chrome session free",
        }
...
def linkedin_read(url: str) -> dict:
    ...
    try:
        ...
        result["note"] = (
            "Free via Jina Reader for public LinkedIn pages. For Profile details, company page, job search, needs OpenCLI Chrome session free - Tell Agent 'help me config LinkedIn'"
        )
        return result
    except Exception as e:
