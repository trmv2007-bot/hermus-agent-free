...
def resolve_imports(mod_path, node):
    ...
    if isinstance(node, ast.Import):
        ...
        # Relative-import resolution is approximate: resolving it exactly
        # requires walking back from the importing package.
        if node.module:
        ...
        for dec in fn.decorator_list:
            if (
                ...
                try:
                    ...
                    path = None
                routes.append(
                    {"method": dec.func.attr.upper(), "path": path, "handler": fn.name, "module": str(f.relative_to(ROOT))}
                )
...
for f in list(iter_py("core")):
    s = f.read_text()
    for pat, label in [
        (r"OPENROUTER|OPENAI|GROQ|OLLAMA|GEMINI|DEEPSEEK|HF_|HUGGINGFACE", "env-key"),
