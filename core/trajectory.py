...
class TrajectoryManager:
    ...
    def batch_generate(self, prompts: list[str], model: str = None, max_workers: int = 3) -> list[dict]:
        ...
        logger.info(f"[Trajectory] Batch generation: {len(prompts)} prompts, {max_workers} workers")
        ...
        try:
            ...
            logger.error(f"[Trajectory] Parallel batch failed: {e}, falling back to sequential")
        ...
        logger.info(f"[Trajectory] Batch generated {len(results)} trajectories, checkpointed to {checkpoint_path}")
        return results
    ...
    def compress_trajectories(self, input_path: str = None, output_path: str = None, max_tokens: int = 4000) -> dict:
        ...
        logger.info(f"[Trajectory] Compressing {len(trajectories)} trajectories to fit {max_tokens} token budget")
        ...
        for traj in trajectories:
            ...
            if "tool_calls" in traj:
                compressed_traj["tool_calls"] = [
                    {"name": tc.get("name", ""), "args": str(tc.get("arguments", ""))[:200]}
            ...
            "parameters": {
                ...
                "properties": {
                    "prompts": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "List of prompts to generate trajectories for",
                    },
            ...
            "parameters": {
                ...
                "properties": {
                    "max_tokens": {
                        "type": "integer",
                        "default": 4000,
                        "description": "Max tokens per trajectory after compression",
                    }
                },
                "required": [],
            },
...
TOOL_MAP = {
    "trajectory_batch_generate": lambda prompts, model=None, max_workers=3: trajectory_manager.batch_generate(
        prompts, model, max_workers
    ),
