...
class Updater:
    ...
    def get_local_commit(self) -> dict:
        ...
        try:
            result = subprocess.run(
                ["git", "rev-parse", "HEAD"], capture_output=True, text=True, timeout=5, cwd=str(config.resolve_path("."))
            )
    ...
    def get_remote_commit(self) -> dict:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                result = subprocess.run(["git", "ls-remote", self.repo_url, "main"], capture_output=True, text=True, timeout=10)
    ...
    def check_for_updates(self) -> dict:
        ...
        if not local_commit or not remote_commit:
            return {"update_available": False, "local": local, "remote": remote, "message": "Could not get commits"}
        ...
        if local_commit == remote_commit:
            ...
            try:
                ...
                subprocess.run(
                    ["git", "fetch", "origin", "main"], capture_output=True, timeout=15, cwd=str(config.resolve_path("."))
                )
                result = subprocess.run(
    ...
    def update(self) -> dict:
        ...
        try:
            ...
            result_pull = subprocess.run(
                ["git", "pull", "origin", "main"], capture_output=True, text=True, timeout=60, cwd=str(config.resolve_path("."))
            )
            ...
            return {
                ...
                "message": f"Updated via git pull - new commit {new_local.get('short')} - {new_local.get('message', '')[:80]}"
                if result_pull.returncode == 0
                else f"Update failed: {result_pull.stderr[:500]}",
            }
        except Exception as e:
...
def get_updater_for_current_repo() -> Updater:
    ...
    try:
        result = subprocess.run(
            ["git", "remote", "get-url", "origin"], capture_output=True, text=True, timeout=5, cwd=str(config.resolve_path("."))
        )
        if result.returncode == 0:
