...
class CouncilSession:
    ...
    def run(self) -> dict:
        ...
        try:
            ...
            logger.info(
                f"[⚖️ Counsel] difficulty={self.difficulty} | max_members={self.max_members} | rounds={self.max_rounds} | execute={self.execute_plan_flag}"
            )
            ...
            logger.error(f"[⚖️ Counsel] Fatal error: {e}")
    ...
    def convene(self):
        ...
        logger.info(f"[⚖️ Counsel] Roster: {describe_roster(self.members)}")
    ...
    def _add_turn(self, agent: str, content: str, round_num: int = 0, extra: dict | None = None):
        turn = {
            "agent": agent,
            "role": "council",
    ...
    def proposals(self):
        ...
        logger.info(f"[⚖️ Counsel] Proposals from {len(self.debaters)} members (parallel)...")
        ...
        with ThreadPoolExecutor(max_workers=max(2, len(self.debaters))) as ex:
            ...
            for f in as_completed(futures):
                ...
                self._add_turn(
                    r["member"], r["content"], round_num=1, extra={"role": r["role"], "model": self._model_for(r["member"])}
                )
        ...
        for round_num in range(2, self.max_rounds + 1):
            ...
            for member in self.debaters:
                ...
                if member.role == "critic" and self.rules.get("critic_must_attach_evidence", True):
                    critic_rule = (
                        "\nYou MUST end with either 'OBJECTION:' and one concrete problem, or 'APPROVAL:' and your reason."
                    )
                messages = self._base_messages(
                    member,
            ...
            critic_turns = [t for t in self.transcript if t.get("round") == round_num and "critic" in t.get("agent", "").lower()]
            if critic_turns:
                ...
                if "APPROVAL:" in c_content or "NO_OBJECTIONS" in c_content or "NO OBJECTIONS" in c_content:
                    logger.info(f"[⚖️ Counsel] Critic approved; consensus reached early in round {round_num}.")
    ...
    def _recent_transcript_text(self, limit: int = 8, max_len: int = 250) -> str:
        return "\n".join(f"[{t['agent']} R{t['round']}]: {t['content'][:max_len]}" for t in self.transcript[-limit:])
                ...
                self.votes = [
                    {
                        ...
                        "reason": parsed["reason"][:400],
                    }
                ]
                self._add_turn(judge.name, json.dumps(parsed)[:800], round_num=99, extra={"model": judge.model})
                logger.info(f"[👤 {judge.name}] winner={parsed['winner']} scores={parsed['scores']}")
                return
        except Exception as e:
            logger.error(f"[⚖️ Counsel] Judge failed ({e}) - skipping formal vote")

    def _parse_vote_json(self, content: str) -> dict | None:
        import re
        ...
        if winner not in self.proposal_texts:
            winner = (
                max(data["scores"], key=lambda k: data["scores"].get(k, 0)) if data["scores"] else next(iter(self.proposal_texts))
            )
    ...
    def finalize_plan(self):
        ...
        if chair:
            ...
            try:
                ...
                if steps:
                    plan = Plan(
                        goal=self.goal, steps=steps, strategy="council", difficulty=self.difficulty, session_id=self.session_id
                    )
            except Exception as e:
                logger.error(f"[⚖️ Counsel] Chair plan parse failed ({e}) - fallback")
        if plan is None:
    ...
    def execute(self):
        if not self.plan or not self.plan.steps:
            logger.warning("[⚖️ Counsel] No plan steps to execute")
            return
        ...
        for idx, step in enumerate(self.plan.steps):
            ...
            messages = [
                ...
                {
                    "role": "user",
                    "content": f"Plan step {idx + 1}/{len(self.plan.steps)}:\nGoal: {step.goal}\nAction: {step.action}\nVerify: {step.verify or 'n/a'}",
                },
            ]
            observations = []
            step_failed = False
                ...
                for tc in resp.tool_calls:
                    ...
                    if isinstance(targs, str):
                        try:
                            ...
                            targs = {}
                    try:
                        logger.info(f"[⚙️ {tname}({json.dumps(targs)[:120]})]")
                        if tname == EXPAND_TOOL_NAME:
                            # The subset missed something this step needs. Widen
                            # for the remaining rounds of this step.
                            tools = list(all_tools)
                            observations.append(f"tool {tname} -> full tool catalog now available")
                            messages.append(
                                {"role": "user", "content": "Full tool catalog is now available. Call the tool you needed."}
                            )
                            continue
            ...
            if failures >= reconvene_threshold and replans < max_replans and idx + 1 < len(self.plan.steps):
                replans += 1
                self.replanned = True
                logger.error(f"[⚖️ Counsel] {failures} failures -> mini-reconvene #{replans} to replan remaining steps")
        ...
        for member in [chair, critic] if critic else [chair]:
            ...
            try:
                ...
                if steps:
                    ...
                    logger.info(f"[⚖️ Counsel] Replanned by {member.name}: {len(steps)} new steps")
                    return
            except Exception as e:
                logger.error(f"[⚖️ Counsel] mini-reconvene failed ({e})")
    ...
    def synthesize(self):
        ...
        if not self.final_answer:
            self.final_answer = f"Council plan:\n{plan_text}\n\nExecution:\n{results_text}"
        ...
        try:
            ...
            def review():
                try:
                    ...
                    logger.error(f"[🕯️ Meta-Counsel] review failed: {e}")
