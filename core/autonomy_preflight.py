...
class AutonomyPreflightReport:
    ...
    def to_markdown(self) -> str:
        ...
        lines += _table(
            self.findings,
            ["tool", "zone", "decision", "red_lines", "capabilities", "description"],
            empty="No likely powerful actions detected.",
        )
        lines += ["", "## Missing approvals", ""]
        lines += _table(
            self.missing_approvals,
            ["tool", "zone", "red_lines", "description", "missing_reason"],
            empty="No missing approvals predicted.",
        )
        lines += ["", "## Missing capabilities/connectors", ""]
        lines += _table(
            self.missing_capabilities,
            ["tool", "expected_capability", "description", "missing_reason"],
            empty="No missing capabilities predicted.",
        )
        lines += ["", "## Red-line blockers", ""]
        lines += _table(
            self.red_line_blocks, ["tool", "red_lines", "description", "reasons"], empty="No red-line blockers predicted."
        )
        lines += ["", "## Suggested approval prompts", ""]
        lines += _table(
            self.suggested_approval_prompts,
            ["title", "tool", "red_lines", "resources", "purpose"],
            empty="No draft approval prompts suggested.",
        )
        ...
        lines += [
            ...
            "## Rule",
            "",
            "Pre-flight is advisory. It does not grant approvals, activate capabilities, or execute tools.",
            "",
        ]
        return "\n".join(lines)
...
def create_preflight_approval_requests(
    ...
    *,
    ...
    for prompt in report.suggested_approval_prompts:
        ...
        safety = {
            "zone": "yellow",
            "red_lines": prompt.get("red_lines", []),
...
def infer_actions(goal: str) -> list[PreflightAction]:
    ...
    if any(w in text for w in ("download", "documents", "home directory", "~/", "/home/", "folder", "local files")):
        ...
        add(
            ...
            "Read-only defensive local folder scan",
            {"path": resource, "purpose": goal, "label": label},
            "read",
        )
    if any(w in text for w in ("malware", "virus", "forensic", "incident")) and not actions:
        add(
            "local_folder_defensive_scan",
            "Read-only defensive local inspection",
            {"path": "approved local folder", "purpose": goal},
            "read",
        )
    if any(
        w in text for w in ("network scan", "port scan", "local network", "subnet", "pentest", "vulnerability", "security scan")
    ):
    ...
    if any(w in text for w in ("gmail", "email", "send message", "reply", "slack", "telegram", "discord", "post as")):
        add(
            "send_email",
            "Delegated communication or external send",
            {"action": "send email/reply as user", "purpose": goal},
            "delegated_communication_connector",
        )
    if any(w in text for w in ("wallet", "trade", "trading", "crypto", "stock", "invest", "spend", "buy", "purchase")):
        add(
            "wallet_trade",
            "Agent wallet/spending/trading action",
            {"action": "agent wallet trade/spend", "purpose": goal},
            "agent_wallet",
        )
    if any(w in text for w in ("calendar", "schedule meeting", "book", "appointment")):
        add(
            "calendar_write",
            "External calendar/account write",
            {"action": "write calendar event", "purpose": goal},
            "calendar_connector",
        )
    ...
    if any(w in text for w in ("code", "build", "test", "fix", "implement", "repository", "app")):
        ...
        add(
            ...
            {"command": "run project tests/build", "purpose": goal},
            "execute_host",
        )
...
def _scope_assumptions(goal: str, findings: list[PreflightFinding]) -> list[str]:
    ...
    if 6 in lines:
        assumptions.append(
            "Financial actions require an isolated agent wallet/account, visible ledger, reserve, and risk limits."
        )
    if 9 in lines:
        assumptions.append(
            "Delegated communication requires approved account/channel, recipient/scope, and high-impact confirmation rules."
        )
...
def _next_steps(
    ...
    if missing:
        steps.append(
            "Record or review missing capabilities in CAPABILITY_LEDGER.md and generate setup proposals before implementation."
        )
    if approvals:
...
def _publish_preflight_prompts(
    goal: str, report: AutonomyPreflightReport, created: list[dict[str, Any]], errors: list[dict[str, Any]]
) -> None:
    try:
        from .events import get_bus
