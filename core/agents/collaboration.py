...
class CollaborativeMission:
    ...
    async def _execute_tasks(self) -> None:
        ...
        while True:
            ...
            async with self._lock:
                all_completed = all(t.status in [TaskStatus.COMPLETED, TaskStatus.FAILED] for t in self.tasks.values())
            ...
            for task_id, task in self.tasks.items():
                if task.status == TaskStatus.PENDING:
                    # Check dependencies
                    deps_met = all(
                        self.tasks[dep].status == TaskStatus.COMPLETED if dep in self.tasks else True for dep in task.dependencies
                    )
    ...
    async def _review(self) -> None:
        ...
        all_success = all(t.status == TaskStatus.COMPLETED for t in self.tasks.values())
