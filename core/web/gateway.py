...
class WebGateway:
    ...
    def __init__(self, config: Any | None = None, *, bus: Any | None = None):
        if config is None:
        ...
        self._page_cache: LRUCache | None = (
            LRUCache(max_size=int(getattr(config, "web_cache_size", 128)), ttl_seconds=int(getattr(config, "web_cache_ttl", 600)))
            if getattr(config, "web_cache_enabled", True)
            else None
        )
    ...
    def fetch(
        ...
        *,
        ...
        if use_cache and self._page_cache is not None and not session_name:
            ...
            cache_key = f"fetch:{strategy}:html={int(include_html)}:md={int(want_markdown)}:{normalized}"
    ...
    def extract_links(self, url: str, *, pattern: str = "", max_links: int = 200, **kw: Any) -> dict[str, Any]:
        result = self.fetch(url, want_markdown=False, **kw)
    ...
    def search_and_extract(
        ...
        *,
        ...
        try:
            ...
            return {"ok": False, "error": f"search failed: {exc}", "error_code": "WEB_SEARCH_FAILED"}
    ...
    def crawl(
        ...
        *,
        ...
        per_domain_delay_ms: int | None = None,
        emit: Any | None = None,
        ...
        for url in urls or []:
            try:
                ...
                return {"ok": False, "error": str(exc), "error_code": "WEB_SECURITY_BLOCKED", "url": redact_url(url)}
        ...
        delay = (
            int(per_domain_delay_ms)
            if per_domain_delay_ms is not None
    ...
    def crawl_async(
        self, urls: list[str], *, session_key: str = "web-crawl", mission_id: str | None = None, **kw: Any
    ) -> dict[str, Any]:
        ...
        try:
            ...
            return {"ok": False, "error": f"job queue unavailable: {exc}", "error_code": "WEB_QUEUE_UNAVAILABLE"}
        return {"ok": True, "queued": True, "job_id": job.id, "run_id": job.run_id, "kind": "web.crawl", "status": "queued"}
    ...
    def _crawl_emit(self, emit: Any | None, mission_id: str | None, run_id: str | None) -> Any:
    ...
    # --------------------------------------------------------------- sessions
    def session_create(self, name: str, domains: list[str], *, strategy: str = "static") -> dict[str, Any]:
        if not self.enabled:
            return {"ok": False, "error": "web subsystem disabled", "error_code": "WEB_DISABLED"}
        ...
        try:
            self._bus.publish(
                EventEnvelope(
                    ...
                    type=event_type,
                    command="web.fetch",
                    target=redact_url(result.final_url or result.url),
                    args_redacted={
                        "strategy": result.strategy,
                        "status_code": result.status_code,
                        "failure_class": result.failure_class,
