...
class GroundedTarget:
    ...
    parent_region: str | None = None  # Parent container
...
class VisualGrounder:
                ...
                if isinstance(box, (list, tuple)) and len(box) >= 4:
                    ...
                    if self._matches_description({"name": label}, description):
                        ...
                        return GroundedTarget(
                            ...
                            bbox=bbox,
                            confidence=float(
                                result.get("confidences", [0.8])[i] if i < len(result.get("confidences", [])) else 0.8
                            ),
    ...
    def _matches_description(self, target_data: dict[str, Any], description: str) -> bool:
        ...
        return (
            desc_lower in name
            or name in desc_lower
            or desc_lower in text
            or any(word in name for word in desc_lower.split() if len(word) > 3)
        )
...
class PreClickVerifier:
    ...
    def verify(
        ...
        result["safe_to_click"] = (
            ...
            and overlap >= self.overlap_threshold
            and current.state in ("visible", "enabled", "unknown")
        )
...
class GroundingSystem:
    ...
    def verify_before_click(
        ...
        frame: Any,
        screen_size: tuple[int, int] = (1920, 1080),
    ) -> tuple[bool, tuple[float, float] | None, dict[str, Any]]:
