...
def register_computer_tools(registry) -> None:
    ...
    def computer_action(action: str, args: dict[str, Any] | None = None, allow_dry_run: bool = False) -> dict[str, Any]:
        if action not in _ACTIONS:
            return {
                "ok": False,
                "error": "unknown_computer_action",
        ...
        if any(v is None for v in kwargs.values()):
            return {"ok": False, "error": "missing_args", "reason": f"action '{action}' requires {schema}"}
        ctl = ComputerActionController()
        cap = ctl.backend_capability()
        if not cap["available"] and not bool(allow_dry_run):
            return {"ok": False, "error": "computer_control_unavailable", "reason": cap["reason"], "capability": cap}
